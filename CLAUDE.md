...
**`MailboxDO` is used two ways.** `idFromName(<mailbox address>)` holds that mailbox's mail; `idFromName("AUTH")` holds users, sessions and mailbox access grants. SQLite via workers-qb, migrations in `durableObject/migrations.ts` (never edit an applied migration — add the next one). Migration SQL must contain no `--` comments: workers-qb concatenates the SQL with its own INSERT and workerd rejects the result with "SQL code did not contain a statement". A migration that throws is not recorded, so it re-runs on the next load and fails differently (`duplicate column name`) — if you see that, the migration is broken, not already applied. Passwords are a single SHA-256 digest (`#hashPassword`).
